...
Modified from Ben Best <https://github.com/marinebon/info-fk/blob/master/prep/prep.Rmd> 
...
### plot_indicators.csv
`svg_id`s are read from plot_indicators and the number plots for each given `svg_id` is shown
...
### *.svg
`svg_id`s are read from svg files & tabulated
```{r svg_elements}
...
### csv_elements.csv
`svg_id`s are read from csv_elements; n is the # of svg files the svg_id is expected to be in

```{r plot_indicators, message=F}
elm_d = read_csv(elements_csv)
...
named elements from the .svg files without a matching entry in plot_indicators
